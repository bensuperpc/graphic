...
def json_linechart_years(file_path: str) -> None:
    ...
    if "key_data_name" not in json_data:
        raise ValueError("Invalid JSON data: Missing 'key_data_name' key")
    ...
    if "save_path" not in json_data:
        raise ValueError("Invalid JSON data: Missing 'save_path' key")
    ...
    key_data_name = json_data["key_data_name"]
    ...
    linechart_years(data, title, xlabel, ylabel, legend_title, key_data_name, x_data_name, y_data_name, point_data_name, source, dark_mode, save_path)
